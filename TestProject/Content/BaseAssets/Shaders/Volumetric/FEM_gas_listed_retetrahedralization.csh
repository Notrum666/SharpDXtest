...
RWStructuredBuffer<int> tetrahedronsCounter : register(u2);
RWStructuredBuffer<MeshVertex> vertices : register(u3);
RWStructuredBuffer<int> toRetetrahedralizeList : register(u4);
...
void main(uint3 groupID : SV_GroupID, uint groupIndex : SV_GroupIndex)
{
    ...
    int shouldBeRetetrahedralized = toRetetrahedralizeList[curOctantIndex];
    if (shouldBeRetetrahedralized != 1)
        return;
    
    toRetetrahedralizeList[curOctantIndex] = 0;
    
    OctreeNode curOctant = octree[curOctantIndex];
    for (int i = curOctant.tetrahedronsStart; i < curOctant.tetrahedronsEnd; i++) // delete current tetrahedrons first, if any
